...
parts = []
for name in ('portraits', 'jobs', 'engine', 'renderer', 'audio', 'main'):
...
(root / 'game/offline.js').write_text(script)
...
html = html.replace('<script src="game/local-entry.js"></script>', '<script>\n' + script + '\n</script>')
...
print('Built game/offline.js for opening index.html directly')
